 ...
 * visual error feedback, and accessible form controls. Users must enter
 * a valid mobile number and ticket reference before accessing the arena.
 */
...
/** Regex pattern to validate international phone numbers (7–15 digits, optional '+' prefix). */
...
/**
 * Validates the mobile number field against required and format rules.
 *
 * @param value - The current mobile number input value.
 * @returns An error message string, or empty string if the value is valid.
 */
function validateMobile(value: string): string {
  const sanitized = sanitizeInput(value);
  if (!sanitized) return 'Mobile number is required.';
  if (!PHONE_PATTERN.test(sanitized.replace(/[\s()-]/g, ''))) {
...
/**
 * Validates the password/ticket ID field against required and length rules.
 *
...
/**
 * TicketLogin provides the entry point for the Kinetic Arena application.
 * It renders a branded login form with two validated fields: mobile number
 * and ticket reference. On successful validation, the user is navigated
 * to the main dashboard.
 *
 * Features:
 * - Real-time input sanitization to prevent XSS.
 * - Visual inline error messages for each field.
 * - Accessible labels, ARIA attributes, and semantic HTML5 form elements.
 * - Keyboard-friendly: pressing Enter submits the form.
 *
...
export default function TicketLogin(): React.JSX.Element {
   ...
   * If validation passes, simulates authentication and navigates to the dashboard.
   * If validation fails, sets inline error messages for each invalid field.
   */
  const handleLogin = useCallback(
    (e: React.FormEvent<HTMLFormElement>) => {
      ...
      // Simulate an authentication delay for realistic UX
      setTimeout(() => {
        setIsSubmitting(false);
        navigate('/dashboard');
      }, 600);
